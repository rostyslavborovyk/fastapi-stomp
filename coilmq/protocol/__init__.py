...
class AsyncSTOMP10(AsyncSTOMP):
    ...
    async def connect(self, frame, response=None):
        ...
        if self.engine.authenticator:
            ...
            if token := frame.headers.get('token'):
                ...
                login = frame.headers.get('login')
                passcode = frame.headers.get('passcode')
                if not await self.engine.authenticator.authenticate(login, passcode):
                    raise AuthError("Authentication failed for %s" % login)
    ...
    async def abort(self, frame):
        ...
        await self.engine.queue_manager.resend_transaction_frames(
            self.engine.connection, frame.transaction)
        del self.engine.transactions[frame.transaction]
    ...
    async def ack(self, frame):
        ...
        if "message-id" not in frame.headers:
            raise ProtocolError("No message-id specified for ACK command.")
        await self.engine.queue_manager.ack(self.engine.connection, frame, id=frame.headers.get("id"))
...
class AsyncSTOMP11(AsyncSTOMP10):

    SUPPORTED_VERSIONS = {'1.0', '1.1'}

    def __init__(self, engine, send_heartbeat_interval=100, receive_heartbeat_interval=100, *args, **kwargs):
        super(AsyncSTOMP11, self).__init__(engine)
        self.last_hb = datetime.datetime.now()
        self.last_hb_sent = datetime.datetime.now()
        self.timer = CoilThreadingTimer()
    ...
    async def send_heartbeat(self):
        # screw it, just send an error frame
        await self.engine.connection.send_frame(ErrorFrame('heartbeat'))
    ...
    async def connect(self, frame, response=None):
        ...
        await super(AsyncSTOMP11, self).connect(frame, response=connected_frame)
    ...
    async def _negotiate_protocol(self, frame, response):
        ...
        if not common:
            ...
            response.headers['version'] = max(common)
            protocol_class = ASYNC_PROTOCOL_MAP[response.headers['version']]
            if type(self) is not protocol_class:
                self.engine.protocol = protocol_class(self.engine)
                self.engine.protocol.connect(frame, response=response)
    ...
    async def ack(self, frame):
        ...
        if "message-id" not in frame.headers:
            raise ProtocolError("No message-id specified for ACK command.")
        await self.engine.queue_manager.ack(self.engine.connection, frame, id=frame.headers["subscription"])


class AsyncSTOMP12(AsyncSTOMP11):

    SUPPORTED_VERSIONS = AsyncSTOMP11.SUPPORTED_VERSIONS.union({'1.2', })
    ...
    async def connect(self, frame, response=None):
        ...
        if host != socket.getfqdn():
            raise ProtocolError('Virtual hosting is not supported or host is unknown')
        await super(AsyncSTOMP12, self).connect(frame, response)
    ...
    async def ack(self, frame):
        ...
        if "message-id" not in frame.headers:
            raise ProtocolError("No message-id specified for ACK command.")
        await self.engine.queue_manager.ack(self.engine.connection, frame, id=frame.headers["id"])


PROTOCOL_MAP = {'1.0': STOMP10, '1.1': STOMP11, '1.2': STOMP12}
ASYNC_PROTOCOL_MAP = {'1.0': AsyncSTOMP10, '1.1': AsyncSTOMP11, '1.2': AsyncSTOMP12}
